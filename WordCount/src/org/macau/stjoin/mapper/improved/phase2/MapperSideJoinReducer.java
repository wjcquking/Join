...
public class MapperSideJoinReducer extends
	Reducer<LongWritable, FlickrValue, Text, Text>{
		...
		public void reduce(LongWritable key, Iterable<FlickrValue> values,
			...
			for(FlickrValue value:values){
				...
				if(key.get() == 0){
					context.write(new Text(fv.getOthers()), new Text(""));
				}else{
			...
			for(java.util.Iterator<Integer> obj = sMap.keySet().iterator();obj.hasNext();){
				
				Integer i = obj.next();
				
				
				if(rMap.containsKey(i+1)){
					
					
					for(int j = 0;j < sMap.get(i).size();j++){
						
						FlickrValue value1 = sMap.get(i).get(j);
						
						//for the same tail, there is no need for comparing
						
						for(int k = 0; k < rMap.get(i+1).size();k++){
							FlickrValue value2 = rMap.get(i+1).get(k);
							tCompareCount++;
							if(FlickrSimilarityUtil.TemporalSimilarity(value1, value2)){
								sCompareCount++;
								if(FlickrSimilarityUtil.SpatialSimilarity(value1, value2)){
