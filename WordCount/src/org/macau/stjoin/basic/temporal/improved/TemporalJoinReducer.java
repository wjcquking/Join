...
public class TemporalJoinReducer extends
	Reducer<LongWritable, FlickrValueWithMultiFeature, Text, Text>{
		
		private final Text text = new Text();
		
		private final Map<Integer,ArrayList<FlickrValueWithMultiFeature>> rMap = new HashMap<Integer,ArrayList<FlickrValueWithMultiFeature>>();
		private final Map<Integer,ArrayList<FlickrValueWithMultiFeature>> sMap = new HashMap<Integer,ArrayList<FlickrValueWithMultiFeature>>();
		...
		public void reduce(LongWritable key, Iterable<FlickrValueWithMultiFeature> values,
			...
			for(FlickrValueWithMultiFeature value:values){
			    ...
			    if(fv.getTag() == FlickrSimilarityUtil.R_tag){
			    	...
			    	if(rMap.containsKey(value.getTileNumber())){
				    	...
				    	rMap.get(value.getTileNumber()).add(new FlickrValueWithMultiFeature(fv));
			    	...
			    	if(sMap.containsKey(value.getTileNumber())){
				    	...
				    	sMap.get(value.getTileNumber()).add(new FlickrValueWithMultiFeature(fv));
				...
				if(sMap.containsKey(i)){
					...
					for(int j = 0;j < rMap.get(i).size();j++){
						...
						for(int k = 0; k < sMap.get(i).size();k++){
							...
							tCompareCount++;
							if(FlickrSimilarityUtil.SingleValueSimilarity(value1.getTimestamp(), value2.getTimestamp(), FlickrSimilarityUtil.TEMPORAL_THRESHOLD)){
								
								sCompareCount++;
								if(FlickrSimilarityUtil.SpatialSimilarity(value1, value2)){
									
									oCompareCount++;
									if(FlickrSimilarityUtil.MultiValueSimilarity(value1.getTiles(), value2.getTiles(), ",", FlickrSimilarityUtil.TEXTUAL_THRESHOLD)){
