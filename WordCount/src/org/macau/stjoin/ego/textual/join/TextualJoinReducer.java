...
Reducer<IntWritable, FlickrValue, Text, Text>{
	...
	public void reduce(IntWritable key, Iterable<FlickrValue> values,
		...
		for(FlickrValue value:values){
			...
			String[] textualList = fv.getTiles().split(";");
			...
			for(int i = textualList.length-1; i >= textualList.length-prefixLength;i--){
				...
				if(value.getTag() == FlickrSimilarityUtil.R_tag){
			    	...
			    	if(rMap.containsKey(tokenID)){
				    	
				    	rMap.get(tokenID).add(new FlickrValue(fv));
				    	...
				    	rMap.put(tokenID, list);
				    	
				    }
			    }else{
			    	
			    	if(sMap.containsKey(tokenID)){
				    	
				    	sMap.get(tokenID).add(new FlickrValue(fv));
